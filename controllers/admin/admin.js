...
exports.approveTrxConfirm = async (db, req, res) => {
    let hash = req.params.hash;
    const response1 = await fetch(config.blockchainApiUrl + 'getTransaction/' + hash, {
        method: 'GET', headers: {
    ...
    if (data1.blockNumber && data1.status == true) {
        // await db.query("UPDATE item SET blockchain_confirmation = 1 WHERE token_hash = ?",[hash]);
        res.status(200).send({
            success: true,
            msg: "Transaction confirmed!!"
        });
    } else if (data1.blockNumber && data1.status == false) {
        //await db.query("UPDATE item SET blockchain_confirmation = 2 WHERE token_hash = ?",[hash]);
        res.status(200).send({
            success: false,
...
exports.mintTrxConfirm = async (db, req, res) => {
    let hash = req.params.hash;
    const response1 = await fetch(config.blockchainApiUrl + 'getTransaction/' + hash, {
        method: 'GET', headers: {
            'Accept': 'application/json',
...
exports.listItem = async (db, req, res) => {
    ...
    var qry = " Select i.id, i.increasing_amount , cast(coalesce(getMaxBid(i.id, i.owner_id),i.price) as decimal) as price1 , i.file_type ,i.name,date_format(i.start_date,'%d %M %Y %H:%i:%s') as start_date, i.description, i.image, i.owner,i.owner_id,i.sell_type,date_format(i.expiry_date,'%d %M %Y %H:%i:%s') as expiry_date, i.item_category_id, i.token_id, coalesce(getMaxBid(i.id, i.owner_id)+i.increasing_amount,i.price) as price , i.is_sold, rarity.icon from item as i LEFT JOIN rarity ON i.rarity = rarity.id where blockchain_confirmation=1 ";
    if (trending === '0' && topselling === '0' && sold === '0') {
        var qry = " Select i.id, i.increasing_amount, cast(coalesce(getMaxBid(i.id, i.owner_id),i.price) as decimal) as price1 , i.file_type, i.name,date_format(i.start_date,'%d %M %Y %H:%i:%s') as start_date, i.description, i.image, i.owner,i.owner_id, i.item_category_id,i.sell_type,date_format(i.expiry_date,'%d %M %Y %H:%i:%s') as expiry_date, i.token_id, coalesce(getMaxBid(i.id, i.owner_id)+ i.increasing_amount ,i.price)  as price , i.is_sold, rarity.icon from item as i LEFT JOIN rarity ON i.rarity = rarity.id where blockchain_confirmation=1   ";
    } else {
    ...
    if (!order_by && order_by == 0 || order_by == undefined) {
...
exports.insertItem = async (db, req, res) => {
    ...
    let blockchain_confirmation = req.body.blockchain_confirmation
...
exports.getItem = async (db, req, res) => {
    ...
    var qry = "SELECT it.*, date_format(it.start_date,'%Y-%m-%d') as startdate,date_format(it.expiry_date,'%Y-%m-%d') as expirydate,   it.id as item_id ,ct.id,ct.name as category_name,coalesce(getMaxBid(it.id,it.owner_id),it.price) as max_bid FROM item as it LEFT JOIN item_category as ct ON ct.id=it.item_category_id WHERE 1 "
...
exports.editItem = async (db, req, res) => {
    // *not using*
    var id = req.body.id;
    var itemname = req.body.itemname;
    var description = req.body.description;
    ...
    await db.query(adminQueries.editItem, [users, id], function (error, data) {
        if (error) {
            return res.status(400).send({
                success: false,
                msg: "Error occured!!",
...
exports.getContact = async (db, req, res) => {
    db.query(adminQueries.getContact, function (error, data) {
        if (error) {
            return res.status(400).send({
                success: false,
                msg: "Error Occured",
...
exports.dropsItem = async (db, req, res) => {
    ...
    //    var orderby = req.body.orderby;
    var qry = " Select i.id, i.increasing_amount , cast(coalesce(getMaxBid(i.id, i.owner_id),i.price) as decimal) as price1 , i.file_type ,i.name, i.description, i.image, i.owner,i.owner_id, i.item_category_id, i.token_id, coalesce(getMaxBid(i.id, i.owner_id)+i.increasing_amount,i.price) as price , i.is_sold, rarity.icon from item as i LEFT JOIN rarity ON i.rarity = rarity.id where blockchain_confirmation=1 ";
    if (trending === '0' && topselling === '0' && sold === '0') {
        var qry = " Select i.id, i.increasing_amount, cast(coalesce(getMaxBid(i.id, i.owner_id),i.price) as decimal) as price1 , i.file_type, i.name, i.description, i.image, i.owner,i.owner_id, i.item_category_id, i.token_id, coalesce(getMaxBid(i.id, i.owner_id)+ i.increasing_amount ,i.price)  as price , i.is_sold, rarity.icon from item as i LEFT JOIN rarity ON i.rarity = rarity.id where blockchain_confirmation=1 ";
    } else {
...
/* -------------------Start Blog  -------------------------*/
exports.createBlog = async (db, req, res) => {
    var user_id = parseInt(req.body.user_id)
    var user_profile = ''
    var created_by = ''
    var image = (!req.files['image']) ? req.body.image : req.files['image'][0].filename;

    await db.query('Select * from users where id = ?', [user_id], async function (err, data) {
        if (err) {
            return res.status(400).send({
                success: false,
                msg: "Error occured!!",
            ...
            // var user_profile = data[0]?.profile_pic
            // var created_by = data[0].user_name !== '' ? data[0].user_name : data[0].full_name
        }
        ...
        console.log('admin dgfd sfsf sdfsd', blog)
        await db.query("INSERT INTO blog set ?", [blog], function (err, data) {
            if (err) {
                res.status(400).send({
                    success: false,
                    msg: "Error occured!!",
                ...
                res.status(200).send({
                    success: true,
                    msg: "Blog Added Successfully Done!",
                    response: data
                });
            }
        });
...
exports.getBlogList = async (db, req, res) => {

    await db.query("Select * from blog ORDER BY blog_id DESC", function (error, data) {
        if (error) {
            return res.status(400).send({
                success: false,
                msg: "Error occured!!",
...
exports.getBlogDetail = async (db, req, res) => {
    var blog_id = parseInt(req.body.blog_id)
    await db.query("SELECT * FROM blog WHERE blog_id=?", [blog_id], function (error, data) {
        if (error) {
            return res.status(400).send({
                success: false,
                msg: "Error occured!!",
...
exports.deleteBlog = async (db, req, res) => {
    var blog_id = parseInt(req.body.blog_id)
    await db.query("DELETE FROM blog WHERE blog_id=?", [blog_id], function (error, data) {
        if (error) {
            return res.status(400).send({
                success: false,
                msg: "Error occured!!",
...
exports.changeBlogStatus = async (db, req, res) => {
    var blog_id = req.body.blog_id
    var is_active = req.body.is_active
    await db.query("UPDATE blog SET is_active=? WHERE blog_id=? ", [is_active, blog_id], function (error, data) {
        if (error) {
            return res.status(400).send({
                success: false,
                msg: "Error occured!!",
            ...
            res.status(200).send({
                success: true,
                msg: `Blog ${is_active === 0 ? 'deactivated' : 'activated'} successfully`,
                response: {}
            });
        }
...
exports.getTransaction = async (db, req, res) => {
    try {
        db.query(adminQueries.getTransaction, function (error, result) {
            if (error) {
                return res.status(400).send({
                    success: false,
                    msg: "error occured",

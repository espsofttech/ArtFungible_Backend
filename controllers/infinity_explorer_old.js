...
exports.listItemexplorer  = async (db,req,res)=>{
    ...
    var qry =" Select i.id,ie.id as item_edition_id, case when length(i.name)>=30 then concat(left(i.name,30),'...')  else i.name end as name,i.name as item_fullname,i.description,i.image,i.file_type,i.owner,i.owner_id,i.created_by,i.item_category_id,i.token_id,ie.price,coalesce(i.start_date,i.datetime) as start_date,i.expiry_date,ie.edition_text,ie.edition_no from item_edition as ie left join item as i on i.id=ie.item_id where  ie.is_sold=0 and ie.id in (select min(id) from item_edition where is_sold=0 group by item_id)  and (date(i.expiry_date) >= CURRENT_DATE or i.expiry_date is null) and i.is_active=1";
...
exports.itemdetails = async (db, req, res) => {
    ...
    const responsefee = await fetch('http://infinity8.io:8001/api/erc1155/getFeeFortransfer', {
        method: 'POST', headers: {
